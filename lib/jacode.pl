...
#
$rcsid =
q$Id: jacode.pl,v 2.13.4.15 branched from jcode.pl,v 2.13 2000/09/29 16:10:05 utashiro Exp $;
$VERSION =
...
#       Japanese code given in the second argument $OUTPUT_encoding.
#       $OUTPUT_encoding can be any of "jis", "sjis", "euc" or "utf8", or
#       use "noconv" when you don't want the code conversion.
#       Input code is recognized automatically from the line
#       itself when $INPUT_encoding is not supplied. It is better
#       to specify $INPUT_encoding, since jacode::getcode's guess
#       is not always right. xxx2yyy routine is more efficient
...
#
# Modern Perl style using CPAN software
#
#   use Modern::Open 0.08;
#   open(FILE,'<:encoding(utf8)','input.txt');
#
######################################################################
...
#---------------------------------------------------------------------
# Returns max value of $_[$[] and $_[$[] w('o')w
#---------------------------------------------------------------------
...
sub getcode {
    ...
    # odd katakana
    elsif (/(^|[\x00-\x7f])$re_odd_kana($|[\x00-\x7f])/go) {
        $matched = 1;
        $code    = 'sjis';
    }
...
=item jacode::convert(\$line, $OUTPUT_encoding [, $INPUT_encoding [, $option]])
  ...
  use "noconv" when you don't want the code conversion.
  Input code is recognized automatically from the line
  itself when $INPUT_encoding is not supplied. It is better to
  specify $INPUT_encoding, since jacode::getcode's guess is not
  ...
  For perl is 5.8.1 or later, jacode::convert acts as a wrapper
  to Encode::from_to. When $OUTPUT_encoding or $INPUT_encoding is neither "jis",
...
Copyright (c) 2010, 2011, 2014, 2015, 2016 INABA Hitoshi E<lt>ina@cpan.org>E<gt>
