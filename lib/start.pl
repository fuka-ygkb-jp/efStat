...
unless (-e $lib_fstat_func) {
	...
	print "<HEAD><META http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"><TITLE>efStat $ver / 異常終了</TITLE></HEAD>\n";
...
### jacode.plを読み込む
unless (-e $lib_jcode) {
	&func::PutError("jacode.pl を読み込むことができませんでした。<BR>位置の指定に誤りがないか確認下さい。");
}
require $lib_jcode;
...
### ログ単体表示モード(Webstat互換)
	...
	else                   { $P{MODE} = 'solo_all';     $html_title .= " / $filename[0] (全て表示)"; }
